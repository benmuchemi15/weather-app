...
var resultTextEl = document.querySelector('#city');
var city;
var weather = [];
var cityList = [];
...
function updateCityList(city) {  //saves the city list to local storage
    cityList.indexOf(city) === -1 ? cityList.push(city) : console.log("City already on list")
    localStorage.setItem("cityList", JSON.stringify(cityList)); //saves cityList
    showCityList(cityList);
}

function searchApi2(varLat, varLon,city) {  //have to search onecal API to get 5 day forcast 
...
function searchApi(query) {
    ...
    fetch(locQueryUrl)
      ...
      .then(function (locRes) {
        ...
        city = query;
        console.log(city + ` located at:`+varLat+`x`+ varLon);
        searchApi2(varLat, varLon, city);
    })
      .catch(function (error) {
        console.error(error);
      });
